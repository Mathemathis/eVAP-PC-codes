...
def create_charging_graph(park_model):
    ...
    for i in park_model.B_bus_trips_id:
        ...
        for j in park_model.B_bus_trips_id:
            if is_parked_together(park_model,i,j):
                Arcs.append((i,j))
                # Arcs.append((j,i))
        # for j in park_model.B_bus_trips_id:
        #     if i<j:
...
def define_outgoing_edges(i, Starts, end):
    return [j for j in range(len(Starts)) if Starts[j]>= end]
