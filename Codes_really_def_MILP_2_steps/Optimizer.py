...
class Planning_model:
    ...
    def create_obj_and_constraints(self):
        ...
        self.define_count_shifts()
        self.define_limit_shifts()
    ...
    def define_count_shifts(self):
    
        def rule_count_shifts(_b, i, l):

            # if (i,66666) in self.model.Arcs_bus:
            #     if l!=1:
            #         return self.model.delta[i,l] - self.model.delta[i,l-1] <= 10000*(self.model.eps[i,l]+self.model.x[i, 66666])
            #     else:
            #         return self.model.delta[i,l] <= 10000*(self.model.eps[i,l]+self.model.x[i, 66666])
            
            # else :
                if l!=1:
                    return self.model.delta[i,l] - self.model.delta[i,l-1] <= 10000*(self.model.eps[i,l])
                else:
                    return self.model.delta[i,l] <= 10000*(self.model.eps[i,l])
        
        self.model.C23 = pyo.Constraint(self.model.B_bus_trips_id, self.model.L, rule = rule_count_shifts)


    def define_limit_shifts(self):
    
        def rule_limit_shifts(_b,):

            sum=0
            for i in self.model.B_bus_trips_id:
                for l in self.model.L:
                    sum+=self.model.eps[i,l]
            
            return sum <= len(self.model.B_bus_trips)*5
        
        self.model.C24 = pyo.Constraint(rule = rule_limit_shifts)
    ...
    def define_constraint_order_shift_1(self):
        
        def rule_order_shift_1(_b,i, l):
            sum = 0
            for (i1, l1, j, k) in self.model.Arcs_driver:
                if i1==i and l1==l:
                    sum+=self.model.beta[i,l, j, k]
            return sum<=self.model.eps[i,l]
        
        self.model.C30 = pyo.Constraint(self.model.B_bus_trips_id, self.model.L, rule = rule_order_shift_1) 
    
    def define_constraint_order_shift_2(self):
            
        def rule_order_shift_2(_b,i, l):
            sum = 0
            for (i1, l1, j, k) in self.model.Arcs_driver:
                if i1==i and l1==l:
                    sum+=self.model.beta[i,l, j, k]
            return self.model.eps[i,l] <= sum+self.model.alpha[i,l]
        
        self.model.C31 = pyo.Constraint(self.model.B_bus_trips_id, self.model.L, rule = rule_order_shift_2) 
...
def deactivate_shift_problem(model):
        for i in model.B_bus_trips_id:
            for l in model.L:
                model.eps[i,l].fixed=True
        model.C23.deactivate()
        model.C24.deactivate()

def activate_shift_problem(model):
        for i in model.B_bus_trips_id:
            for l in model.L:
                model.eps[i,l].fixed=False
                if l != len(model.L):
                    model.delta[i,l].fixed=False
        for (i,j) in model.Arcs_bus:
            model.y[i,j].fixed=True
        
        for i in model.B_bus_trips_id:
            # model.s[i].fixed=True
            model.q_[i].fixed=True
            model._q[i].fixed=True
            model.t[i].fixed=True
            model.a[i].fixed=True
        model.C23.activate()
        model.C24.activate()
        model.C3.deactivate()
        model.C4.deactivate()
        ...
        model.C6.deactivate()
        model.C7.deactivate()
        model.C10.deactivate()
        model.C11.deactivate()
        model.C15.activate()
        model.C15bis.activate()
        model.C17.activate()
        model.C18.activate()
        obj=sum([model.eps[i,l] for i in model.B_bus_trips_id for l in model.L])
        model.obj = pyo.Objective(expr= obj , sense=pyo.minimize)

def activate_shift_problem_bis(model):
        print(len(model.B_bus_trips_id)* len(model.L))
        for i in model.B_bus_trips_id:
            for l in model.L:
                model.eps[i,l].fixed=False
        
        for (i,j) in model.Arcs_bus:
            model.y[i,j].fixed=False

        # print(len(model.Arcs_bus))
        
        for i in model.B_bus_trips_id:
            # model.s[i].fixed=False
            model.q_[i].fixed=False
            model._q[i].fixed=False
            model.t[i].fixed=False
            model.a[i].fixed=False
