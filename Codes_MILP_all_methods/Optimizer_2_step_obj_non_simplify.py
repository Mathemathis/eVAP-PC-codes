...
class Planning_model:
    def __init__(self, Blocks, EBs, type, rates, SOCmax, SOCmin, C, nb_parking_lanes, nb_spaces):
        ...
        self.model.Arcs_bus = pyo.Set(initialize=Parameters.define_all_arc_bus(init_block+Trips+[Puit], int(15000*len(EBs_id))))
        Arcs_init, park_init = Parameters.define_arc_park_init_optimal(init_block, nb_parking_lanes, nb_spaces)
        self.model.Arcs_park = pyo.Set(initialize=Parameters.define_all_arc_park(init_block+Trips+[Puit], 40000*self.model.V)+ Arcs_init)
