...
def launch_planning(args, time_limit):
    ...
    Planning=Optimizer_global_obj.Planning_model(Block, EBs[:NUM_BUS], 'e2', rates,SOCmax, SOCmin, NUM_CHG, nb_parking_lanes, nb_spaces)
    ...
    parent_dir = "data\\"+str(NUM_BUS) + "_bus\\" + str(NUM_TRIPS_PER_EB) + "_trips_per_bus\\global_obj"
    ...
    logpath = os.path.join(path, 'step_global.log')
    ...
    write(excel_path, Planning.model, 'Global', exit=exit1, time=time1)

    if exit1 not in ['Time limit', 'infesasible']:
        Planning_exit = Results.Results_planning(Planning.model, step='global')
