...
import pandas
import Parameters
import Optimizer_2_step_obj_non_simplify
import Results
import Vehicle
...
def launch_planning(args, time_limit):
    ...
    Planning=Optimizer_2_step_obj_non_simplify.Planning_model(Block, EBs[:NUM_BUS], 'e2', rates,SOCmax, SOCmin, NUM_CHG, nb_parking_lanes, nb_spaces)
    ...
    parent_dir = "data\\"+str(NUM_BUS) + "_bus\\" + str(NUM_TRIPS_PER_EB) + "_trips_per_bus\\2_steps_obj_non_simplify"
    ...
    Optimizer_2_step_obj_non_simplify.deactivate_charging_problem(Planning.model)
    ...
    if do_step_2:
        ...
        while i <len(NUM_CHG) and flag:
            ...
            Optimizer_2_step_obj_non_simplify.deactivate_assignment_problem(Planning.model)
            Optimizer_2_step_obj_non_simplify.deactivate_parking_problem(Planning.model)
            Optimizer_2_step_obj_non_simplify.activate_charging_problem(Planning.model, i+1)
